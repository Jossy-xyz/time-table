...
interface VenueSlotSelectorProps {
  isOpen: boolean;
  onClose: () => void;
 ...
 * Features:
 * - Search/filter venues by code or name
 * - Multi-select checkboxes
 * - Scrollable venue list
 * - Visual selection indicators
 * - Confirm/Cancel buttons
 */
const VenueSlotSelector: React.FC<VenueSlotSelectorProps> = ({
  isOpen,
  onClose,
  onConfirm,
  availableVenues,
  ...
  return (
    <>
      ...
      >
        <div className="bg-surface rounded-institutional shadow-2xl border border-brick/10 pointer-events-auto w-full max-w-md max-h-[80vh] flex flex-col animate-fadeInUp">
          {/* Header */}
          ...
          <div className="px-6 py-4 border-b border-brick/10 flex-shrink-0">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-page border border-brick/10 rounded-institutional text-sm font-bold text-institutional-primary focus:outline-none focus:ring-2 focus:ring-brick/20 transition-all"
                autoFocus
              />
            </div>
          </div>
                  ...
                  {filteredVenues.map((venue, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-institutional-muted line-clamp-2">
                          {venue.name}
                        </div>
                      </div>
                    </motion.label>
            ...
            {selectedVenues.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-2">
                {selectedVenues.map((code) => (
                  <span
                    key={code}
                    className="px-2 py-1 bg-brick/20 border border-brick/30 rounded text-xs font-bold text-brick"
                  >
                    {code}
                  </span>
                ))}
          ...
          {/* Footer Actions */}
          <div className="px-6 py-4 border-t border-brick/10 flex gap-3 justify-end flex-shrink-0">
            <button
              onClick={handleCancel}
...
export default VenueSlotSelector;
